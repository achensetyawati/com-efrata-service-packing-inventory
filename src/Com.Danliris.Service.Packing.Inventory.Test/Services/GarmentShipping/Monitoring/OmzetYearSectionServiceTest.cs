...
{
    public class OmzetYearSectionServiceTest
    {
        ...
        protected OmzetYearSectionService GetService(IServiceProvider serviceProvider)
        {
            return new OmzetYearSectionService(serviceProvider);
        }

        [Fact]
        public void GetReportData_Success()
        {
            var packingListModel = new GarmentPackingListModel("", "", "DL", 1, "", DateTimeOffset.Now, "", "", DateTimeOffset.Now, "", 1, "", "", "", "", "", DateTimeOffset.Now, DateTimeOffset.Now, DateTimeOffset.Now, false, false, "", "", "", null, 1, 1, 1, 1, null, "", "", "", "", "", "", "", false, false, 1, "", GarmentPackingListStatusEnum.CREATED, "", false, "", false, false, false, "", "", 1)
            {
                Id = 1
            };
            ...
            var invoiceModel = new GarmentShippingInvoiceModel(1, "invoiceno", DateTimeOffset.Now, "from", "to", 1, "Sectioncode", "Sectionname", "consignee", "lcno", "issuedby", 1, "sectioncode", "shippingper", DateTimeOffset.Now, "confNo", 1, "staff", 1, "cottn", 1, "mandiri", 10, "", DateTimeOffset.Now, "", DateTimeOffset.Now, "", "", invoiceItemModels, 1000, 1000, "23", "dsdsds", "memo", false, "", DateTimeOffset.Now, "", DateTimeOffset.Now, "", DateTimeOffset.Now, null, 100000, "aa", "aa", invoiceUnitModels, "aa", "aa")
            {
                Id = 1,
            ...
            httpMock.Setup(s => s.GetAsync(It.IsAny<string>()))
                ...
                {
                    Content = new StringContent(JsonConvert.SerializeObject(new { data = new List<Section> { new Section() } }))
                });
